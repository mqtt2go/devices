...
struct info device[] = {{"temperature", 0, "C", "f"}, {"humidity", 0, "%", "i"}, {"switch", 1, "", "s"}};
...
void reportValues(){
  dht.read();
  createReport((char*) "periodic_report", device[DEV_TEMP].name, dht.getTemperatureC(), device[DEV_TEMP].unit);
  serializeJson(doc, buffer);
  getTopic(device[DEV_TEMP].name, (char*) "out");
  client.publish(topic_buff, buffer);

  createReport((char*) "periodic_report", device[DEV_HUM].name, dht.getHumidity(), device[DEV_HUM].unit);
  serializeJson(doc, buffer);
  getTopic(device[DEV_HUM].name, (char*) "out");
  client.publish(topic_buff, buffer);
}
